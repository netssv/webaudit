...
Ultra-Optimized with maximum code reuse and minimal duplication
"""
...
# Custom CSS for better contrast and clean UI
st.markdown("""
<style>
    ...
    /* Sidebar toggle button - Main styling */
...
def display_ai_analysis(results):
    """Display AI Analysis with comprehensive summary for ChatGPT"""
    # Add specific styling for better contrast in AI Analysis
    st.markdown("""
    <style>
        /* AI Analysis specific styling for better contrast */
        .stExpander {
            background-color: #ffffff !important;
            border: 1px solid #d1d5db !important;
            border-radius: 0.5rem !important;
        ...
        /* Ensure markdown in expanders is readable */
    ...
    # Create comprehensive summary instead of raw data
    summary = generate_comprehensive_summary(domain, audit_results, active_modules)
    
    # Create analysis prompt with the comprehensive summary
    prompt = f"""Please analyze this comprehensive website audit summary for {domain}:

{summary}

Based on this detailed audit information, please provide:

1. **Executive Summary** - Overall website health assessment and priority score (1-10)
2. **Critical Issues** - Immediate problems requiring attention with implementation priority
3. **SEO Strategy** - Specific recommendations for:
   - Meta tags optimization with exact character counts
   - Content improvements based on current analysis
   - Technical SEO fixes with implementation steps
   - Social media integration enhancements
4. **Performance Optimization** - Actionable improvements with expected impact
5. **Security & Infrastructure** - SSL, DNS, and server configuration recommendations
6. **Marketing Integration** - Analytics, tracking, and conversion optimization
7. **90-Day Action Plan** - Prioritized roadmap with specific metrics and timelines

Please provide specific, actionable recommendations with clear implementation steps and expected outcomes."""
    ...
    with st.expander("📊 Audit Summary (What ChatGPT will analyze)", expanded=False):
        st.markdown(summary)
    ...
    with col1:
        st.info(f"🎯 **Comprehensive Analysis Ready: {domain}**\n\nDetailed audit summary prepared for AI analysis ({prompt_length:,} chars).")
    ...
    with st.expander("📋 View & Copy Full Prompt", expanded=False):
        st.markdown("**Complete prompt for ChatGPT:**")
        st.code(prompt, language="text")
        
        # Copy to clipboard functionality
        if st.button("📋 Copy Complete Prompt to Clipboard"):
            st.success("✅ Prompt copied! You can now paste it into ChatGPT.")
    
    # Usage instructions
    st.markdown("""
    ---
    **💡 How to get the best AI analysis:**
    1. **Click the button** above to open ChatGPT with the complete prompt
    2. **Review the analysis** - ChatGPT will provide specific recommendations
    3. **Ask follow-ups** - Request details on specific sections or implementations
    
    **🎯 What makes this analysis powerful:**
    - Comprehensive audit data organized for AI understanding
    - Specific metrics and measurements for accurate recommendations
...
def generate_comprehensive_summary(domain, audit_results, active_modules):
    """Generate an optimized comprehensive summary for ChatGPT analysis"""
    try:
        summary_parts = []
        
        summary_parts.append(f"# {domain} Website Audit Analysis")
        summary_parts.append(f"**Analyzed:** {', '.join(active_modules)}")
        summary_parts.append("")
        
        # Collect key metrics across all modules for a concise overview
        key_metrics = {}
        critical_issues = []
        
        # SEO Analysis Summary - Optimized
        if 'seo_marketing' in audit_results:
            seo = audit_results['seo_marketing']
            summary_parts.append("## 📈 SEO Analysis")
            
            # Essential SEO data only
            title = seo.get('title', 'Missing')
            meta_desc = seo.get('meta_description', 'Missing')
            title_len, desc_len = len(title), len(meta_desc)
            
            # Compact format
            summary_parts.append(f"- **Title:** {title[:50]}{'...' if len(title) > 50 else ''} ({title_len} chars)")
            summary_parts.append(f"- **Meta Desc:** {meta_desc[:60]}{'...' if len(meta_desc) > 60 else ''} ({desc_len} chars)")
            
            # Score summary
            overall_score = seo.get('overall_score') or seo.get('seo_score', 'N/A')
            if overall_score != 'N/A':
                summary_parts.append(f"- **SEO Score:** {overall_score}")
            
            # Category scores - compact format
            if 'category_scores' in seo and seo['category_scores']:
                scores = []
                for cat, score in list(seo['category_scores'].items())[:4]:  # Top 4 categories
                    scores.append(f"{cat.replace('_', ' ').title()}: {score}")
                if scores:
                    summary_parts.append(f"- **Categories:** {' | '.join(scores)}")
            
            # Top 3 critical SEO issues only
            if 'issues' in seo and seo['issues'] and isinstance(seo['issues'], list):
                try:
                    top_issues = [issue for issue in seo['issues'][:3] if isinstance(issue, str)]
                    if top_issues:
                        summary_parts.append(f"- **Top Issues:** {' | '.join(top_issues)}")
                        critical_issues.extend(top_issues[:2])
                except:
                    pass
            
            # Social media - simplified
            if 'social_media_links' in seo and seo['social_media_links']:
                try:
                    platforms = list(seo['social_media_links'].keys())[:3]
                    if platforms:
        ...
        # Performance Analysis Summary - Optimized
        if 'performance' in audit_results:
            ...
            if 'response_time' in perf:
                ...
                if hasattr(rt, 'total_seconds'):
                    response_time = f"{rt.total_seconds():.2f}s"
                else:
                    response_time = f"{rt:.2f}s" if isinstance(rt, (int, float)) else str(rt)
                perf_metrics.append(f"Response: {response_time}")
            
            # File size - compact
            if 'total_size' in perf:
                perf_metrics.append(f"Size: {perf['total_size']}")
            
            if perf_metrics:
                summary_parts.append(f"- **Metrics:** {' | '.join(perf_metrics)}")
            
            # Top 2 performance issues only
            if 'issues' in perf and perf['issues'] and isinstance(perf['issues'], list):
                try:
                    top_issues = [issue for issue in perf['issues'][:2] if isinstance(issue, str)]
                    if top_issues:
                        summary_parts.append(f"- **Issues:** {' | '.join(top_issues)}")
                        critical_issues.extend(top_issues)
                except:
                    pass
            
            summary_parts.append("")
        
        # Security Analysis Summary - Compact
        if 'ssl' in audit_results:
            ssl = audit_results['ssl']
            summary_parts.append("## 🔒 Security")
            
            ssl_status = "✅ Active" if ssl.get('has_ssl', False) else "❌ Missing"
            summary_parts.append(f"- **SSL:** {ssl_status}")
            
            # Certificate info - essential only
            if 'certificate_info' in ssl:
                cert_info = ssl['certificate_info']
                cert_details = []
                if 'issuer' in cert_info:
                    cert_details.append(f"Issuer: {cert_info['issuer']}")
                if 'expires' in cert_info:
                    cert_details.append(f"Expires: {cert_info['expires']}")
                if cert_details:
                    summary_parts.append(f"- **Certificate:** {' | '.join(cert_details)}")
            
            # Top 2 security issues only
            if 'issues' in ssl and ssl['issues'] and isinstance(ssl['issues'], list):
                try:
                    top_issues = [issue for issue in ssl['issues'][:2] if isinstance(issue, str)]
                    if top_issues:
                        summary_parts.append(f"- **Issues:** {' | '.join(top_issues)}")
                        critical_issues.extend(top_issues)
                except:
        ...
        if 'dns' in audit_results:
            ...
            if 'a_records' in dns and dns['a_records']:
                dns_info.append(f"A Records: {len(dns['a_records'])}")
            if 'mx_records' in dns and dns['mx_records']:
                dns_info.append(f"MX Records: {len(dns['mx_records'])}")
            
            if dns_info:
                summary_parts.append(f"- **Records:** {' | '.join(dns_info)}")
            
            # Top 2 DNS issues only
            if 'issues' in dns and dns['issues'] and isinstance(dns['issues'], list):
                try:
                    top_issues = [issue for issue in dns['issues'][:2] if isinstance(issue, str)]
                    if top_issues:
                        summary_parts.append(f"- **Issues:** {' | '.join(top_issues)}")
                        critical_issues.extend(top_issues)
                except:
        ...
        # Authority & Ranking Summary - Compact
        if 'ranking' in audit_results:
            ranking = audit_results['ranking']
            summary_parts.append("## 📊 Authority")
            
            authority_metrics = []
            if 'domain_authority' in ranking:
                authority_metrics.append(f"DA: {ranking['domain_authority']}")
            if 'page_authority' in ranking:
                authority_metrics.append(f"PA: {ranking['page_authority']}")
            
            if authority_metrics:
                summary_parts.append(f"- **Metrics:** {' | '.join(authority_metrics)}")
            
            # Top 2 insights only
            if 'insights' in ranking and ranking['insights'] and isinstance(ranking['insights'], list):
                try:
                    top_insights = [insight for insight in ranking['insights'][:2] if isinstance(insight, str)]
                    if top_insights:
                        summary_parts.append(f"- **Insights:** {' | '.join(top_insights)}")
                except:
                    pass
            
            summary_parts.append("")
        
        # Priority Issues Summary - Ultra Compact
        summary_parts.append("## 🎯 Critical Issues")
        
        # Use the collected critical issues from all modules
        unique_issues = list(dict.fromkeys(critical_issues))  # Remove duplicates while preserving order
        if unique_issues:
            # Top 5 most critical issues only
            for i, issue in enumerate(unique_issues[:5], 1):
                if len(issue) > 80:  # Truncate long issues
                    issue = issue[:77] + "..."
        ...
        # Fallback summary in case of any unexpected errors
        error_msg = str(e)[:100]
        return f"""# {domain} Audit Summary
**Modules:** {', '.join(active_modules)}

⚠️ **Error generating summary:** {error_msg}

**Basic Info:**
- Domain: {domain}
- Modules: {', '.join(active_modules)}

Please analyze this website based on available information."""
...
def main():
    ...
    if st.session_state.dark_mode:
        ...
        <style>
            ...
            /* Sidebar toggle button - Dark mode - Enhanced contrast */
            button[data-testid="collapsedControl"] {
                background-color: #ffffff !important;
                color: #1f2937 !important;
        ...
        <style>
            ...
            /* Sidebar toggle button - Light mode - Enhanced contrast */
            button[data-testid="collapsedControl"] {
                background-color: #1f2937 !important;
                color: #ffffff !important;
                border: 2px solid #1f2937 !important;
            ...
            /* Enhanced sidebar styling for light mode */
            .stSidebar {
                background-color: #f9fafb !important;
            ...
            /* Sidebar text and elements for light mode */
            .stSidebar .stMarkdown, 
            .stSidebar .stText, 
            .stSidebar .stCaption, 
            .stSidebar .stSubheader, 
